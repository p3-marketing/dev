...
function ppp_image_attachment($html, $id, $alt, $title, $align, $size) {
	...
	$html = '<amp-img src="'.$image_source[0].'" class="' . $class . '" '.$hwstring.' id="'.esc_attr($id).'" alt="'.esc_attr($alt).' '.esc_attr($title).' | P3.MARKETING" title="'.esc_attr($title).' | P3.MARKETING" layout="responsive"></amp-img>';
    return $html;
}
add_filter('get_image_tag', 'ppp_image_attachment', 10, 6);
